...
def register_socketio_events(socketio_instance):
    ...
    def handle_connect():
        ...
        try:
            ...
            # Default connection for other roles
            logger.info(f'User {user_id} connected with role: {user.role.value if user else "unknown"}')
            emit('connected', {'user_id': user_id, 'message': 'Connected to notification service'})
