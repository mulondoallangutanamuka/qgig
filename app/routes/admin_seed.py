"""
Admin seeding endpoint for environments without shell access
"""
...
admin_seed_blueprint = Blueprint('admin_seed', __name__)
...
@admin_seed_blueprint.route('/admin/seed-data', methods=['POST'])
def seed_data():
    """Seed the database with initial data. Protected endpoint."""
    
    # Simple protection - check for a secret key or admin session
    # In production, you might want to add more robust authentication
    secret_key = request.headers.get('X-Seed-Key')
    if secret_key != os.getenv('SEED_SECRET_KEY', 'seed-me-please'):
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                ...
                "users": {
                    "admin": f"{admin_email} / {admin_password}",
                    "institution": f"{institution_email} / {institution_password}",
                    "professional": f"{professional_email} / {professional_password}"
                },
                "jobs_created": len([j for j in sample_jobs if j["title"] not in existing_titles])
            })
